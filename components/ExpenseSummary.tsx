...
export default function ExpenseSummary({ totalAmount, itemCount }: ExpenseSummaryProps) {
  return (
    <motion.div
      ...
      className="panel-dark relative overflow-hidden p-6 ring-1 ring-white/5"
    >
      ...
      <div className="relative">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-white/76">{itemCount} visible expense{itemCount === 1 ? '' : 's'}</p>
          </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <div className="rounded-xl border border-white/14 bg-white/10 p-4 shadow-inner backdrop-blur">
            <p className="text-xs font-bold uppercase tracking-wider text-white/68">Items</p>
            <p className="mt-2 text-2xl font-bold text-white">{itemCount}</p>
          </div>
          <div className="rounded-xl border border-white/14 bg-white/10 p-4 shadow-inner backdrop-blur">
            <p className="mt-0 inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-emerald-300">
